...
### 2.3. 백엔드 원칙
- **Atlas Data API 사용:** Cloudflare Workers 환경의 제약으로 인해 네이티브 MongoDB 드라이버를 직접 사용하지 않습니다. 대신, 표준 HTTPS 요청(`fetch`)을 통해 **MongoDB Atlas Data API**와 통신하여 모든 데이터베이스 CRUD 작업을 수행합니다.
...
- **트랜잭션 처리:** 골드 소모, 재료 사용 등 여러 데이터 변경이 동시에 일어나는 `Upgrade`(강화, 합성 등)와 같은 작업은 데이터 정합성을 보장해야 합니다. (참고: Atlas Data API는 멀티-도큐먼트 트랜잭션을 지원하지 않으므로, 애플리케이션 레벨에서 데이터 정합성을 신중하게 관리해야 합니다.)
